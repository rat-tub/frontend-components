...
<div class="bg-[#f3e7fb] card rounded-lg overflow-hidden shadow-xl dark:custom-shadow-black py-5 border border-[rgb(237,210,255)] w-72 dark:border-slate-700 cursor-pointer hover:scale-110 transition-transform duration-300 ease-in-out dark:bg-[#272c3a]">
  <div class="flex items-center justify-center">
    <img src="https://images.pexels.com/photos/1040881/pexels-photo-1040881.jpeg?cs=srgb&dl=pexels-sindre-fs-1040881.jpg&fm=jpg" alt={imageAlt} class="rounded-full w-32 h-32 object-cover border border-[#e5c6fc] dark:border-[#121928]" />
  </div>
  <div class="flex flex-col justify-center items-center">
    ...
    <div class="flex gap-5 py-2">
      <a href={github} target="_blank"><Icon src={BsGithub} className="h-5 w-5" title="Custom icon params" /></a>
      <a href={linkedin} target="_blank"> <Icon src={BsInstagram} className="h-5 w-5" title="Custom icon params" /></a>
      <a href={instagram} target="_blank"> <Icon src={BsLinkedin} className="h-5 w-5" title="Custom icon params" /></a>
    </div>
  </div>
</div>
...
<div class="fixed inset-0 flex justify-center items-center bg-black bg-opacity-70 transition-opacity duration-300 opacity-0 pointer-events-none" id="largerCard">
  <div class="max-w-xl bg-white p-8 rounded-lg shadow-lg">
    ...
    <div class="text-center p-3">
      ...
      <h2 class="text-lg font-medium dark:text-slate-400 text-zinc-600 py-3">{role}</h2>
    ...
    <p class="p-12 text-center">{quote}</p>
